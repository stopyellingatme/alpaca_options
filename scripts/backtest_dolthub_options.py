#!/usr/bin/env python3
"""Backtest with DoltHub Historical Options Data.

This script runs a backtest using FREE historical data from DoltHub:
- Real underlying price data from Alpaca
- Real historical options chains from DoltHub (2019-2024)
- No synthetic data generation

DoltHub provides:
- 2,098 symbols with options data
- Coverage: 2019-01-01 to 2024-12-31
- Full option chains with Greeks, IV, bid/ask, volume, OI
- Completely FREE
...
async def run_dolthub_backtest():
    """Run backtest with DoltHub historical options data."""
    ...
    console.print(Panel.fit(
        "[bold cyan]DoltHub Historical Options Backtest[/bold cyan]\n"
        "Using FREE real historical data from DoltHub\n"
        "Coverage: 2019-2024, 2,098 symbols",
        border_style="cyan"
    ))
    ...
    # Backtest parameters
    symbol = "QQQ"
    start_dt = datetime(2024, 2, 1)  # February 2024
    end_dt = datetime(2024, 11, 30)  # November 2024
    ...
    # Initialize fetchers
    console.print("\n[bold]Initializing Data Fetchers...[/bold]")

    # Alpaca for underlying data
    api_key = os.environ.get("ALPACA_API_KEY", "")
    api_secret = os.environ.get("ALPACA_SECRET_KEY", "")

    if not api_key or not api_secret:
        console.print("[red]ERROR: Alpaca credentials required for underlying data![/red]")
    ...
    console.print("[green]✓ Alpaca fetcher initialized (for underlying data)[/green]")

    # DoltHub for options chains
    try:
        dolthub_fetcher = DoltHubOptionsDataFetcher()
        console.print("[green]✓ DoltHub fetcher initialized (for options chains)[/green]")
    except RuntimeError as e:
        console.print(f"[red]ERROR: {e}[/red]")
        console.print("\n[yellow]To install Dolt CLI:[/yellow]")
        console.print("  macOS: brew install dolt")
        console.print("  Linux: curl -L https://github.com/dolthub/dolt/releases/latest/download/install.sh | sudo bash")
        console.print("  Windows: Download from https://github.com/dolthub/dolt/releases/latest")
        console.print("\n[dim]See DOLTHUB_SETUP.md for detailed instructions[/dim]")
        return

    # Fetch underlying data from Alpaca
    console.print("\n[bold]Loading Underlying Data from Alpaca...[/bold]")
    with console.status("[cyan]Fetching price bars..."):
    ...
    console.print("\n[bold yellow]Fetching Historical Options Chains from DoltHub...[/bold yellow]")
    console.print("[dim]Querying free historical database (2019-2024)...[/dim]")

    options_data = {}

    with console.status("[cyan]Fetching DoltHub chains...") as status:
        # Fetch chains at daily intervals
        ...
        for i, timestamp in enumerate(daily_timestamps):
            ...
            if chain:
                ...
                console.print(f"[yellow]  {timestamp.date()}: No DoltHub data available[/yellow]")

    if not options_data:
        console.print("[red]ERROR: No DoltHub options data available for this period![/red]")
        console.print("[yellow]Available dates:[/yellow]")
        available = dolthub_fetcher.get_available_dates(symbol, start_dt, end_dt)
        if available:
            console.print(f"[dim]  First: {available[0].date()}[/dim]")
            console.print(f"[dim]  Last: {available[-1].date()}[/dim]")
            console.print(f"[dim]  Count: {len(available)} dates[/dim]")
        else:
            console.print(f"[red]  No data for {symbol} in DoltHub database[/red]")
        return

    console.print(f"\n[green]✓ Loaded {len(options_data):,} historical option chains from DoltHub[/green]")

    # Create strategy instance
    strategy = VerticalSpreadStrategy()

    # Configure strategy for real market data
    ...
    # Strategy parameters (real market data)
    strategy._config["min_iv_rank"] = 30
    strategy._config["min_dte"] = 21
    strategy._config["max_dte"] = 45
    strategy._config["close_dte"] = 14

    # DoltHub has real open interest data (unlike Alpaca historical)
    strategy._config["min_open_interest"] = 50  # Standard liquidity filter

    # Allow reasonable spreads for real market data
    strategy._config["max_spread_percent"] = 10.0

    # Realistic return on risk for credit spreads
    strategy._config["min_return_on_risk"] = 0.10  # 10% minimum
    ...
    console.print(f"[dim]  Min Open Interest: {strategy._config.get('min_open_interest', 50)}[/dim]")
    console.print(f"[dim]  Max Spread %: {strategy._config.get('max_spread_percent', 10.0)}[/dim]")
    console.print(f"[dim]  Min Return on Risk: {strategy._config.get('min_return_on_risk', 0.10)*100:.1f}%[/dim]")

    # Create backtest engine
    engine = BacktestEngine(settings.backtesting, settings.risk)

    # Run backtest
    console.print("\n[bold]Running Backtest with DoltHub Data...[/bold]")
    ...
    perf_table = Table(title="Performance Summary (DoltHub Data)", box=box.ROUNDED)
    ...
    console.print(f"[green]  ✓ Options Chains: DoltHub FREE Database (REAL 2019-2024)[/green]")
    console.print(f"[green]  ✓ Greeks: From DoltHub (REAL)[/green]")
    console.print(f"[green]  ✓ Implied Volatility: From DoltHub (REAL)[/green]")
    console.print(f"[green]  ✓ Open Interest: From DoltHub (REAL)[/green]")

    console.print("\n[bold cyan]Key Insights:[/bold cyan]")
    console.print(f"[cyan]  • This backtest uses 100% REAL historical market data[/cyan]")
    console.print(f"[cyan]  • DoltHub provides free, complete options chains[/cyan]")
    console.print(f"[cyan]  • Results are more accurate than synthetic backtests[/cyan]")
    console.print(f"[cyan]  • Still expect 10-20% slippage in live trading vs backtest[/cyan]")

    console.print("\n[dim]Backtest with DoltHub historical data completed successfully.[/dim]")

    return result


if __name__ == "__main__":
    asyncio.run(run_dolthub_backtest())
