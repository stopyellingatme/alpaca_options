#!/usr/bin/env python3
"""Parallelized Delta Target Optimization for Vertical Spread Strategy.

This version runs ALL symbol/delta combinations concurrently for maximum speedup.

Performance Improvement:
- Sequential: 8-12 hours for 4 symbols × 5 deltas = 20 backtests
- Parallel: 2-3 hours (4x speedup, limited by CPU cores)

Tests delta targets: 0.15, 0.18, 0.20, 0.22, 0.25

Usage:
    uv run python scripts/optimization/optimize_delta_parallel.py
    uv run python scripts/optimization/optimize_delta_parallel.py --symbol AAPL  # Single symbol
    uv run python scripts/optimization/optimize_delta_parallel.py --quick  # 2023-2024 only
"""
...
async def main():
    """Run delta optimization with parallel execution."""
    parser = argparse.ArgumentParser(description="Optimize delta target for vertical spreads (PARALLEL)")
    parser.add_argument(
        "--symbol",
        type=str,
        default=None,
    ...
    console.print(Panel.fit(
        "[bold cyan]Delta Target Optimization (PARALLEL)[/bold cyan]\n"
        "Testing delta values: 0.15, 0.18, 0.20, 0.22, 0.25\n"
        "Baseline: 0.20 (20 delta)\n\n"
        "[bold green]Parallelization: Running ALL symbol/delta combinations concurrently[/bold green]",
        border_style="cyan"
    ))
    ...
    total_backtests = len(symbols) * len(delta_targets)
    ...
    tasks = []
    task_metadata = []  # Track which symbol/delta each task represents

    for symbol in symbols:
        for delta in delta_targets:
            task = run_delta_backtest(
                delta_target=delta,
                symbol=symbol,
                start_dt=start_dt,
                end_dt=end_dt,
            ...
            tasks.append(task)
            task_metadata.append({"symbol": symbol, "delta": delta})
    ...
    with Progress(
        ...
        # Run all tasks concurrently (use return_exceptions to handle errors gracefully)
        results = []
        completed = 0
        ...
        for coro in asyncio.as_completed(tasks):
            try:
                ...
                console.print(f"\n[red]Error in {meta['symbol']} delta={meta['delta']}: {e}[/red]")
                results.append({
                    "symbol": meta["symbol"],
                    "delta": meta["delta"],
                    "error": str(e)
            ...
            completed += 1
            progress.update(task_progress, advance=1, description=f"[cyan]Completed {completed}/{len(tasks)} backtests...")
    ...
    # Sort each symbol's results by delta
    for symbol in all_results:
        all_results[symbol].sort(key=lambda r: r["delta"])

    # Display results (same as original script)
    console.print("\n\n")
    console.print(Panel.fit(
