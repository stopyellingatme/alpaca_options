#!/usr/bin/env python3
"""DTE Range Optimization for Vertical Spread Strategy.

This script tests different DTE (Days To Expiration) ranges to find the optimal balance between:
- Trade frequency (shorter DTE = more opportunities)
- Theta decay capture (longer DTE = more premium)
- Gamma risk management (earlier exit = less risk)

Tests:
- Entry DTE ranges: (14,30), (21,45), (30,60)
- Exit DTE: 7, 14, 21

Usage:
    uv run python scripts/optimize_dte.py
    uv run python scripts/optimize_dte.py --symbol SPY  # Test single symbol
    uv run python scripts/optimize_dte.py --quick  # Use 2023-2024 only
"""
...
async def run_dte_backtest(
    ...
    """Run backtest with specific DTE parameters.
    ...
    # Configure strategy with specific DTE parameters
    ...
    # Set DTE PARAMETERS (optimization parameters)
    ...
    strategy._config["delta_target"] = 0.20
...
async def main():
    """Run DTE optimization across multiple combinations."""
    parser = argparse.ArgumentParser(description="Optimize DTE ranges for vertical spreads")
    parser.add_argument(
        "--symbol",
        type=str,
        default=None,
    ...
    console.print(Panel.fit(
        "[bold cyan]DTE Range Optimization[/bold cyan]\n"
        "Testing entry ranges: (14-30), (21-45), (30-60) DTE\n"
        "Testing exit DTEs: 7, 14, 21\n"
        "Baseline: Entry (21-45), Exit 14",
        border_style="cyan"
    ))
    ...
    for symbol in symbols:
        ...
        test_combinations = []
        for min_dte, max_dte in entry_ranges:
            for close_dte in exit_dtes:
                if close_dte >= min_dte:  # Skip invalid combinations
                    continue
                test_combinations.append((min_dte, max_dte, close_dte))
        ...
        with Progress(
            ...
            task = progress.add_task(
                f"[cyan]Testing DTE combinations for {symbol}...",
                total=len(test_combinations)
            )

            for min_dte, max_dte, close_dte in test_combinations:
                progress.update(
                    task,
                    description=f"[cyan]Testing entry ({min_dte}-{max_dte}), exit {close_dte}..."
                )

                try:
                    result = await run_dte_backtest(
                        min_dte=min_dte,
                        max_dte=max_dte,
                        close_dte=close_dte,
                        symbol=symbol,
                    ...
                    console.print(
                        f"\n[red]Error testing ({min_dte}-{max_dte}), exit {close_dte} "
                        f"for {symbol}: {e}[/red]"
                    )
    ...
    console.print("\n\n")
    console.print(Panel.fit(
        "[bold green]DTE OPTIMIZATION RESULTS[/bold green]",
        border_style="green"
    ))
    ...
    for symbol, results in all_results.items():
        ...
        table = Table(title=f"{symbol} DTE Optimization", box=box.ROUNDED)
        table.add_column("Entry Range", justify="center", style="cyan", width=12)
        table.add_column("Exit DTE", justify="center", style="cyan", width=10)
        ...
        for result in results:
            if "error" in result:
                ...
                m = result["metrics"]
                baseline = (
                    result["min_dte"] == 21 and
                    result["max_dte"] == 45 and
                    result["close_dte"] == 14
                )

                # Highlight baseline
                entry_str = f"{result['min_dte']}-{result['max_dte']}"
                exit_str = str(result['close_dte'])
                if baseline:
                    entry_str = f"[bold]{entry_str}*[/bold]"
                    exit_str = f"[bold]{exit_str}*[/bold]"
        ...
        if valid_results:
            ...
            console.print(
                f"\n[green]  ✓ Best Sharpe: Entry ({best_sharpe['min_dte']}-{best_sharpe['max_dte']}), "
                f"Exit {best_sharpe['close_dte']} "
                f"(Sharpe {best_sharpe['metrics'].sharpe_ratio:.2f})[/green]"
            )

            # Best by trade frequency (most trades with decent Sharpe)
            frequent_results = [r for r in valid_results if r["metrics"].sharpe_ratio > 1.0]
            if frequent_results:
                most_trades = max(frequent_results, key=lambda r: r["metrics"].total_trades)
                console.print(
                    f"[green]  ✓ Most Active (Sharpe > 1.0): Entry ({most_trades['min_dte']}-{most_trades['max_dte']}), "
                    f"Exit {most_trades['close_dte']} "
                    f"({most_trades['metrics'].total_trades} trades)[/green]"
                )

            # Baseline comparison
            baseline_result = next(
                (r for r in valid_results if r["min_dte"] == 21 and r["max_dte"] == 45 and r["close_dte"] == 14),
                None
            )
            if baseline_result and (
                best_sharpe["min_dte"] != 21 or
                best_sharpe["max_dte"] != 45 or
    ...
    console.print("[dim]* = Current baseline (Entry 21-45, Exit 14)[/dim]\n")
    ...
    for symbol, results in all_results.items():
        ...
        if valid_results:
            ...
            if baseline and (
                ...
                console.print(
                    f"[green]✓ {symbol}: Current DTE ranges are optimal[/green]"
                )

    console.print("\n[dim]DTE optimization complete.[/dim]")
